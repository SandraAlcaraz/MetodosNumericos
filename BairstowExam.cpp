...
void Bairstow(float r, float s, float array[], int grado){
    ...
    float errorR = abs(deltaR/r)*100; //ERROR PARCIAL2
    float errorS = abs(deltaS/s)*100; //ERROR PARCIAL2
    ...
    while (errorR >= .0001 || errorS >= .0001){ //ERROR PARCIAL2
        b[0]= array[0];
        b[1] = array[1]+(b[0]*r);
        b[2] = array[2]+(b[1]*r)+(b[0]*s);
        for (int i = 3; i < grado+1; ++i)
    ...
    float number = pow(r,2)+(4*s);
    cout << "NUMBER" << number << endl;
    if(number >= 0){
    	float x1 = (r+sqrt(number))/2;
    	float x2 = (r-sqrt(number))/2;
    	cout << "----RAICES-----" << endl;
	    cout << "x1: " << x1 << endl;
	    cout << "x2: " << x2 << endl;
    }else{
